...
def _make_grid(nx=20, ny=20):
        # Creates a rectangular grid out of 2 given 1-dimensional arrays
...
def split_for_trace_model(pred = None, anchor_grid = None):
    ...
    for i in range(3):
        ...
        y = pred[i].sigmoid() # squishes any real number into a range between 0 and 1
        gr = _make_grid(nx, ny).to(pred[i].device)
        y[..., 0:2] = (y[..., 0:2] * 2. - 0.5 + gr) * st[i]  # xy
        y[..., 2:4] = (y[..., 2:4] * 2) ** 2 * anchor_grid[i]  # wh
        z.append(y.view(bs, -1, 85)) # append list of 85 in last position
    pred = torch.cat(z, 1) # Concatenation
    return pred
